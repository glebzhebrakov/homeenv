...
public class ClassifiedImagesResource {
    ...
    @RequestMapping(
            value = Constants.RestApi.Classification.CLASSIFICATIONS_BY_CLASS,
            method = RequestMethod.GET,
            produces = MediaType.APPLICATION_JSON_VALUE
    )
    public ResponseEntity<List<ClassifiedImageDTO>> findClassified(
            @PathVariable(name = "class") String cls,
            @PathVariable(name = "from") Integer from,
            @PathVariable(name = "to") Integer to)
    {
        return new ResponseEntity<>(classificationService.findClassifiedByClass(cls,from, to)
                .stream()
                .map(ClassifiedImageDTO::new)
                .collect(Collectors.toList()), HttpStatus.OK
        );
